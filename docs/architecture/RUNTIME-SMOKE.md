...
The tightened Case J and new Case K deterministic preflights PASS. Authenticated reruns remain pending because the Codex usage quota was exhausted on 2026-09-25. Revised Case J reached installed `prepareExecutionWithProvenance()`, artifact-backed Implementer-owned README mutation, one reported worker artifact, and linked Lead spawn/completion records; the JSON stream exposed no native worker identifier, so the identity remains framework-logical. The run failed before lightweight verification/final completion/audit after a sandboxed nested `git` child-process call returned `EPERM`, then the usage limit ended the turn. Case K was blocked at turn start by the same usage limit. The earlier Case J authenticated PASS covered the previous Lead-direct Tier 0 contract and is retained only as historical evidence, not as attestation of the tightened contract. Runtime Event records what actually happened; Decision Provenance records why Hybrid chose the control-flow action; Actor Artifact records what an individual worker did; Audit checks whether the recorded decisions and actions agree. Each layer stores bounded metadata only and never hidden reasoning.
