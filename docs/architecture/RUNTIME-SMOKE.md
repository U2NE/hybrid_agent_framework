...
- **Routing probe:** exercises Luna high/xhigh/max request acceptance plus local fail-closed rejection of an invalid/unapproved model; the invalid probe never reaches the Codex endpoint and no model-less fallback is attempted.
...
The tightened Case J and parallel Case K deterministic preflights PASS, and both current authenticated contracts now PASS. Case J uses an explicit Luna/medium outer Lead and Implementer route, completes the Tier 0 worker-owned README mutation, lightweight verification, completion, and clean audit without Lead target mutation. Case K uses an explicit Luna/medium outer Lead and two policy-routed Luna/medium Implementers; one parallel parent links two disjoint children, both spawn events precede the first completion, both reported actor artifacts match exact file ownership, and the audit is clean. Runtime Event records what happened; Decision Provenance records why Hybrid chose the control-flow action; Actor Artifact records bounded worker activity; Audit checks agreement. These records attest the explicit model/effort requests accepted by the runtime, not an independently observed backend serving identity.
...
Run `npm run smoke:decision:preflight` / `npm run smoke:parallel-provenance:preflight` for deterministic installed checks. The corresponding `:live` commands require authenticated Codex capacity. On 2026-09-25 the revised Case J and Case K authenticated executions both completed with exit zero and their persisted artifacts pass the current semantic validators with clean audits.
