...
`recovered_task_completed` is recovery-only. A new record of that kind must be backed by a completed durable worktree integration journal for the exact run/revision/descriptor/task/attempt/lease, and the recorded base HEAD plus final main-workspace hash must still match. `ExecutionRunStore.recoverTaskLease()` is the normal path; direct recovered-terminal commit is independently fenced by the same integration validation.
