...
- sealed execution isolation mode (`current-workspace` or `worktree`);
...
`hybrid-exec-graph/v4` seals the deterministic scheduler `isolationMode` into every executable node. Lease request, durable lease, task contract, and dispatch authorization carry the same mode, so a caller cannot switch a task between worktree and current-workspace after sealing. A supplied isolation plan is accepted only when it exactly matches a fresh deterministic scheduler calculation for the same tasks and runtime availability inputs.

For a `worktree` authorization, **every successful terminal** (`task_completed` or `recovered_task_completed`) must match completed durable integration evidence for the exact run/revision/descriptor/task/attempt/lease, including the recorded base HEAD and final main-workspace hash. Worker return or a detached patch alone cannot authorize completion. `recovered_task_completed` is recovery-only and is invalid for `current-workspace`; `ExecutionRunStore.recoverTaskLease()` is the normal recovery path, while direct terminal commits are independently fenced by the same isolation/integration checks.

The transition must be durably present in `TRANSITIONS.jsonl`, carry at least one evidence reference, and match the exact lease run, descriptor hash, lease, graph revision, task, attempt, and effect policy. A **new terminal transition requires the active durable dispatch authorization for that exact task attempt**. The authorization is validation input only: its lease token and full capability/task contract are never copied into the transition ledger. Terminal records persist only the sealed graph `descriptorHash` and non-secret `leaseId` as first-class authority identity. Before persistence, terminal commit holds the graph-revision fence, verifies that authorization against its persisted graph revision and durable lease, then validates descriptor hash, lease ID, revision, executable agent node, attempt, and effect policy. Missing, forged, mismatched, or inactive authority fails closed before the terminal record enters the ledger. A released authorization may only replay the exact terminal transition already persisted for it; it cannot create a different terminal record after release. Transition commits are serialized by a durable per-run `.transitions.lock` and persisted by atomic ledger replacement, so separate Lead/store instances cannot race an unchecked append. For each `(descriptorHash, graphRevision, task, attempt)`, at most one terminal outcome may exist; a competing `task_completed`, `recovered_task_completed`, or `task_aborted_reconciled` is fenced before persistence. Existing ledgers containing multiple terminal outcomes fail closed as corrupt. Recovery also requires the exact current descriptor/revision binding, so a completion from an older graph revision never marks the current revision complete. `ExecutionRunStore.releaseTaskLease()` reads that persisted transition, constructs a `hybrid-lease-release-proof/v1` bound to the lease descriptor and transition fingerprint, and only then calls the low-level lease store.
