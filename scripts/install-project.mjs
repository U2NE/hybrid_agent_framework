...
const HYBRID_ROLES = Object.freeze({
  ...
  'browser-functional-tester': 'Independent browser functional QA tester',
  'browser-adversarial-reviewer': 'Independent adversarial browser break-it QA reviewer',
...
export function mergeAgentsInstructions(content) {
  ...
  const block = [
    ...
    '- For interactive UI behavior, activate Browser Functional QA when browser semantics are acceptance-relevant; Tier 2/3 or high-regression-risk interactive work also activates Browser Adversarial QA. Browser QA remains read-only with respect to repository files and executes real Chromium interaction through `.hybrid/core/browser/index.mjs` / `hybrid browser-qa`: explicit scenarios or safe same-origin control discovery, bounded clicks/fills/navigation, screenshot/runtime telemetry, destructive controls disabled unless explicitly authorized. Browser acquisition is raw evidence only; the final Verifier must consume the exact browser `evidenceId` before PASS.',
    '- Serialize same-file writers and respect task dependencies. Multiple concurrent mutating siblings use worktree isolation. The scheduler-selected isolation mode is sealed in execution graph v4 and copied into the durable lease/task contract/dispatch authorization; a supplied isolation plan that differs from deterministic scheduler output fails closed. When scheduler isolation selects worktree mode, the lead binds every writer to `hybrid-worktree-owner/v2` with non-secret run/revision/descriptor/task/attempt/lease identity and owns create → isolated cwd → observed patch handoff → deterministic durable integration queue → verification → cleanup via `.hybrid/core/worktree/index.mjs`. Integration order is task-ID deterministic, not worker-completion order; concurrent integrators must serialize on the durable queue, and unexplained restart/workspace state fails closed for reconciliation. A detached observed patch or worker return alone is not successful-completion or lease-release authority. Both `task_completed` and `recovered_task_completed` under worktree isolation require the same durable dispatch authorization plus a completed integration journal for the exact attempt/lease whose recorded base HEAD and final main-workspace hash still match; otherwise the lease stays active for integration or reconciliation. When a mutating task runs in current-workspace mode, after lease acquisition run `hybrid workspace-guard begin` before spawn and `hybrid workspace-guard complete` after the worker returns; after guard completion, commit a durable terminal `task_completed` transition with evidence and release the lease through that transition; `recovered_task_completed` is recovery-only and must come through `recoverTaskLease()` after completed worktree integration evidence is revalidated; guard completion alone is not release authority. `WRITE_SET_VIOLATION` or `WORKSPACE_BASE_MOVED` requires reconciliation and MUST NOT auto-expand the task write set.',
    '- Before spawning any mutating task, acquire its durable pre-execution resource lease from the current sealed graph through `.hybrid/core/leases/index.mjs` (or `node .hybrid/bin/hybrid.mjs lease acquire ...`). A spawn without a current active dispatch authorization is not permitted. Lease conflicts fail closed; never bypass them by launching another worker. Release only through `ExecutionRunStore.releaseTaskLease()` (or `hybrid lease release`) using a durable terminal transition created with the task dispatch authorization, carrying non-secret `leaseId` plus sealed `descriptorHash`, matching the exact descriptor/lease/revision/task/attempt/effect policy, and having evidence. New terminal creation requires the durable lease to still be active; a released authorization may only replay its exact existing terminal. Never persist the lease token or full authorization in the transition ledger. Terminal commit validates authority and graph identity before persistence; stale-revision completion is not recovery authority. Transition commits are cross-process serialized through the durable run ledger; only one terminal outcome is permitted per descriptor/revision/task/attempt, and competing terminal IDs must fail closed. Raw token-only/null-result release is forbidden. For an abandoned attempt, use `hybrid lease abort` with reconciliation evidence so `task_aborted_reconciled` is durably committed before release. Initial run graph binding, graph advancement, and lease acquisition share the lease-store revision fence; concurrent conflicting initial descriptors fail closed with one winner. Do not advance to a child execution graph while any lease on the current graph remains active. If a worker discovers an additional non-material semantic resource requirement, use `hybrid lease extend`: it never mutates the live authorization in place. Active old-revision leases yield `drain-required`; complete them with a durable terminal transition or reconcile them through `hybrid lease abort`, then evidence-bound release them, retry the extension to publish the child graph, and acquire a fresh attempt on that child. File/read/write contract changes or material product/API/schema/scope/security changes must use the material revision approval path.',
    ...
    '- Actor artifact file fields are role-accurate: record files only read or inspected in `inspectedFiles`, and record only files actually changed by that actor in `modifiedFiles`. Read-only reviewers/testers/verifiers MUST use `modifiedFiles: []` and MUST NOT put inspected-only paths in legacy `files`; `files` is compatibility-only for older modified-file claims.',
    '- Each implementation worker writes one bounded completion self-report only to its own actor artifact with actual changed paths in `modifiedFiles`; worker self-reports remain `reported`. Use an observed native worker ID when available, otherwise explicitly distinguish a framework-logical worker ID.',
