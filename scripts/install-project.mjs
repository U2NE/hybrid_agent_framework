...
export function mergeAgentsInstructions(content) {
  ...
  const block = [
    ...
    '- Serialize same-file writers and respect task dependencies. Multiple concurrent mutating siblings use worktree isolation. When scheduler isolation selects worktree mode, the lead owns create → isolated cwd → observed patch handoff → deterministic durable integration queue → verification → cleanup via `.hybrid/core/worktree/index.mjs`. Integration order is task-ID deterministic, not worker-completion order; concurrent integrators must serialize on the durable queue, and unexplained restart/workspace state fails closed for reconciliation. When a mutating task runs in current-workspace mode, after lease acquisition run `hybrid workspace-guard begin` before spawn and `hybrid workspace-guard complete` after the worker returns; accept completion/release the lease only after that succeeds. `WRITE_SET_VIOLATION` or `WORKSPACE_BASE_MOVED` requires reconciliation and MUST NOT auto-expand the task write set.',
    '- Before spawning any mutating task, acquire its durable pre-execution resource lease from the current sealed graph through `.hybrid/core/leases/index.mjs` (or `node .hybrid/bin/hybrid.mjs lease acquire ...`). A spawn without a current active dispatch authorization is not permitted. Lease conflicts fail closed; never bypass them by launching another worker. Release only after completion/reconciliation evidence is recorded. Do not advance to a child execution graph while any lease on the current graph remains active; graph advancement and acquisition share the lease-store revision fence. If a worker discovers an additional non-material semantic resource requirement, use `hybrid lease extend`: it never mutates the live authorization in place. Active old-revision leases yield `drain-required`; reconcile/release them, retry the extension to publish the child graph, then acquire a fresh attempt on that child. File/read/write contract changes or material product/API/schema/scope/security changes must use the material revision approval path.',
