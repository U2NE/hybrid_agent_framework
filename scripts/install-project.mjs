...
export function mergeAgentsInstructions(content) {
  ...
  const block = [
    ...
    '- Close an integrated implementation snapshot through the generic `runQualityClosure()` exported by `.hybrid/core/orchestrator/index.mjs`; it owns defect repair, proof-gap reassessment, optional shared-context cache reuse, passive observability, default Lead-owned decision/event persistence, and final completion gating without adding a new agent role.',
    ...
    '- Central action-bearing events must use the Lead-owned orchestration event writer (or Lead provenance session). Public low-level `appendRuntimeEvent()` is passive-only and cannot write decision/action ownership records.',
