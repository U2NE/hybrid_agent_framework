...
Independent tasks in the same wave are parallel-eligible sibling workers. Tasks with dependencies run in later waves. Same-file writers are serialized. Multiple concurrent mutating siblings use worktree isolation; generated outputs, migrations, formatters, lockfiles, or low ownership confidence also require worktree isolation when available, otherwise safe serialization. When worktree mode is selected, the lead owns the lifecycle through the Hybrid worktree bridge: create one detached worktree per writer, run each worker with that worktree as cwd, collect only declared-owner patches, fail closed on integration conflict, verify the integrated main workspace, then remove/prune every temporary worktree. When current-workspace mode is selected for a mutating task, acquire its durable resource lease, begin the repository-global current-workspace mutation guard before spawn, and complete the guard after the worker returns. Guard completion alone is not release authority: record a durable terminal `task_completed`/`recovered_task_completed` transition carrying guard/verification evidence, then release through `ExecutionRunStore.releaseTaskLease()` / `hybrid lease release`. For an abandoned attempt, reconcile first and use `hybrid lease abort` so `task_aborted_reconciled` is committed before release. `WRITE_SET_VIOLATION` or `WORKSPACE_BASE_MOVED` stops for reconciliation; never widen ownership after the fact.
