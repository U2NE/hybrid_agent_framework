...
If a running worker discovers that it needs an additional non-material semantic resource, do not widen its active lease or authorization. Request `hybrid lease extend` from the Lead. A `drain-required` result means all active old-revision attempts must be completed/reconciled or explicitly aborted and their leases released; retry the extension, then redispatch under a fresh attempt on the published child graph. `retry-required` means another graph revision won the race, so recompute from the latest graph. Never use lease extension to add files/read-write scope or to bypass material revision approval.
